...
function initTimeline() {
    ...
    years.forEach((year) => {
        ...
        // Click event for expandable content
        yearItem.addEventListener('click', (e) => {
            e.stopPropagation();
            const isCurrentlyShowing = experienceContent.classList.contains('show') && 
                                       experienceContent.dataset.currentYear === year;
            ...
            // Toggle content - close if already showing, open if not
            if (isCurrentlyShowing) {
                hideExperienceContent();
            } else {
                yearItem.classList.add('active');
                showExperienceContent(year);
            }
        });
    ...
    if (years.length > 0) {
        ...
        const firstYearItem = document.querySelector(`.year-item[data-year="${firstYear}"]`);
        if (firstYearItem) {
            firstYearItem.classList.add('active');
            showExperienceContent(firstYear);
        }
    }
}

// Show experience content
function showExperienceContent(year) {
    const experienceContent = document.getElementById('experienceContent');
    ...
    if (experiences.length === 0) {
        experienceContent.innerHTML = '<div class="experience-content-item"><p>此年份無相關經驗</p></div>';
        experienceContent.dataset.currentYear = year;
    ...
    experiences.forEach(exp => {
        ...
        card.className = 'experience-content-item';
        card.innerHTML = html;
    ...
    setTimeout(() => {
        experienceContent.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
    }, 100);
}
...
function initFuture() {
    ...
    futureItems.forEach(item => {
        ...
        item.addEventListener('click', (e) => {
            e.stopPropagation();
            const isCurrentlyShowing = futureContent.classList.contains('show') && 
                                       futureContent.dataset.currentTerm === term;
            
            // Remove active class from all future items
            document.querySelectorAll('.future-item').forEach(futureItem => {
                futureItem.classList.remove('active');
            });
            ...
            if (isCurrentlyShowing) {
                ...
                showFutureContent(term);
            }
        });
    });
    
    // Auto-expand first future item
    if (futureItems.length > 0) {
        const firstItem = futureItems[0];
        const firstTerm = firstItem.dataset.term;
        if (firstTerm) {
            firstItem.classList.add('active');
            showFutureContent(firstTerm);
        }
    }
}

// Show future content
function showFutureContent(term) {
    const futureContent = document.getElementById('futureContent');
    ...
    contentItem.className = 'future-content-item';
    contentItem.innerHTML = html;
    
    futureContent.innerHTML = '';
    futureContent.appendChild(contentItem);
    futureContent.dataset.currentTerm = term;
    futureContent.classList.add('show');
    
    // Smooth scroll to content
    setTimeout(() => {
        futureContent.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
    }, 100);
}

// Hide future content
function hideFutureContent() {
    const futureContent = document.getElementById('futureContent');
    futureContent.classList.remove('show');
    // Remove active class from all future items
